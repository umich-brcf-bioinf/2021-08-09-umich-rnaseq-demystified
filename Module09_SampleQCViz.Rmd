...
![](./images/wayfinder_08.png){width=75%}
...
**TODO: Add image of PCA to walk through interpretation before going through code or have open local file**

In the plot we'll generate below, principle component 1 (PC1) explains ~80% of the variance in our data while principle component 2 (PC2) explains ~12% of the variance. We also see that samples in both `control` groups are fairly tightly grouped, while samples within each `Tx` group do not cluster as tightly. 
...
# NOTE, is this correct? In the text below 'Gtype.Tx' is mentioned...
```
...
It can sometimes be useful to also generate a PCA plot for the raw data as well as the normalized data, especially if ...
